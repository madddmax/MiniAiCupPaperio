...
namespace MiniAiCupPaperio
{
    public static class Simulator
    {
        ...
        public static PlayerModel GetNext(PlayerModel my, string direction, int depth, int deltaDepth)
        {
            var myNext = (PlayerModel) my.Clone();

            for (int i = 1; i <= deltaDepth; i++)
            {
                ...
                foreach (var e in Enemies)
                {
                    if (depth >= 1 &&
                        myNext.Lines.Any(l =>
                            Math.Abs(l[0] - e.Position[0]) + Math.Abs(l[1] - e.Position[1]) <=
                            World.Width * (depth - deltaDepth + i + 2)))
                    {
                        // страх пересечения шлейфа
                        return null;
                    }

                    if (depth >= 1 && myNext.Lines.Length > 0 &&
                        Math.Abs(x - e.Position[0]) + Math.Abs(y - e.Position[1]) <= World.Width * (depth - deltaDepth + i + 2))
                    {
